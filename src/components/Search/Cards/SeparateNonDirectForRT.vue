<template>
    <v-card>
        <v-card-title>{{$t('meta.separateNonDirectForRTCard')}}
            <v-spacer></v-spacer>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapGetters(['getCheapestLiveNonDirectOutboundForRT', 'getCheapestLiveNonDirectInboundForRT', 'getSearchType'])
        },
        mounted() {
            let outboundFlight = this.getCheapestLiveNonDirectOutboundForRT();
            let inboundFlight = this.getCheapestLiveNonDirectInboundForRT();
            if (outboundFlight) {
